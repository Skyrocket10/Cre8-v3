...
import { LIMITS, type Collection, type SceneNode } from '@/lib/document/types';
...
function BindControls({ node, collection }: { node: SceneNode; collection: Collection }) {
  ...
  const setBinding = (prop: string, key: string) =>
    useEditor.getState().transact(key ? 'Bind to a field' : 'Unbind', (draft) => {
      const scene = draft.nodes[node.id];
      if (!scene) return;
      if (key) scene.bind = { ...(scene.bind ?? {}), [prop]: key };
      else if (scene.bind) {
  ...
  return (
    <>
      ...
      {offered.map((prop) => (
        <StyleRow key={prop} label={prop === 'text' ? 'Text' : prop}>
          <Select
            className="flex-1"
            value={node.bind?.[prop] ?? ''}
            options={[
