...
| `stress` | The primitives at their limits: how far each page of the stress template scrolls sideways, whether a word with no break opportunity can be made to wrap, whether three clamped cards come out the same height, which order five boxes are drawn in, where a jump lands, and whether the same document built to read right to left mirrors — measured on one deliberately lopsided box and then across every page of the gallery, which is every block in the registry. The one suite here that needs no Worker: it generates the site and reads it back over `file://` |
...
| `values` | E1, E3, E4 and E5's falsifications, driven rather than seeded: can somebody *say* `when Price is over Budget` in the panel — pick the field from the When… menu, change the operator through its chip, point the value chip at another field through the chevron inside it — and does the published file then paint one row and not the other, on two listings with the **same price** and different budgets, which is the pair no constant can separate. Plus the half that makes it affordable: the file ships no `data-cre8-test`, no `data-cre8-vals` and no script, because a comparison between two fields folds. Then E3: point a reference at another collection through the `→` chip, watch the byline print the author's name on the canvas and in the file, **delete the author record**, and watch it fall back to the copy the designer typed with the record id nowhere in the markup. Then E4, checked in the order it breaks: count a collection that is **empty at that moment** and watch it publish `0` rather than the placeholder, then add a row and watch the same binding say `1`. Then E5: build `⟨Price⟩ ÷ ⟨Rooms⟩` through the chips and check the same sum on the canvas and in the file, on two rows that divide differently |
...
| `republish` | Does the live site follow its records with nobody pressing Publish, does a second publish write nothing, and do the two things that must *not* republish stay put. And the sequence a person performs every day: edit a published project *in the editor*, where the edit travels the socket and the room's copy of the document is whatever immer last produced, then write a record — does the alarm publish it, and does it manage that on the first attempt rather than after a retry storm nothing records |
...
| `inspector` | Does an inspector edit reach the element it was showing rather than the one selected next, does inline text survive a click elsewhere, and does an open overlay actually scope what gets inserted. Every check involves two elements, because one cannot see this class of bug. And, since the panel became as long as what the element uses: does a heading arrive with two sections rather than fifteen, does Add offer the rest grouped in plain words while offering nothing that cannot apply or is already there, does adding one put it on screen and does what is typed into it reach the element, does a section holding a value come back without being asked, does removing one take the declarations with it in a single undo, does a container get the sections a container is about without being asked the four questions it used to be asked on sight, and is a multi-selection style-only
