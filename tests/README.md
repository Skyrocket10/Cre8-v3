...
| `menus` | Right-click: does the menu open where the pointer is, stay on screen — submenus included — keep a multi-selection, close without selecting what it closed over. Does Duplicate from the menu do exactly what ⌘D does, down to the undo step. And in the inspector, is the menu about the *control* you clicked: reset that padding, copy that value onto something else, and leave a text field's own menu alone |
